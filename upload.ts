...
const upload = async () => {
  const data = fs.readFileSync('repo_list.txt', 'utf8');
  ...
  for (let i = 0; i < urls.length; i++) {
    try {
      ...
      const clone_command = `git clone ${url}`
      // await runCommand(clone_command);

      const origin_repo_name = match ? match[1] : ''

      const open_command = `cd ${origin_repo_name}`

      const new_repo_name = origin_repo_name
        .replace(/[_\s.]+/g, '-')
        // .toLowerCase()
        // .replace(/^-/, '')
        // .replace(/-{2,}/g, '-');
